            ...
            <div class="card my-2 bg-light ">
                <div class="card-header" style="background-color:#008F7A">
                    <div class="text-center">
                        <h3 class="text-white">La liste des commandes</h3>
                    </div>
                </div>
                ...
                <table id="mytable" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach($data as $item)
                                <tr class="text-center">
                                    <td data-label="ID">{{$item->id_commande}}</td>
                                    ...
                                    <td data-label="Status" class="{{$item->status==0 ? 'bg-success':'bg-warning'}}">{{$item->status==0 ?'nouveau':'à livraison'}}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            @foreach($data as $item)
                                <tr class="text-center">
                                    ...
                                    <td data-label="Quantité commandée">{{$item->quantite_commande}}</td>
                                    <td data-label="Prix du commande en (DH)"> {{$item->prix_commande}} DH</td>
                                    <td data-label="Date du commande"> {{$item->date_commande}}</td>
                                    <td data-label="Actions" class="d-flex justify-content-center align-items-center">
                                        <!-- <a href="{{route('commande.show',$item->id_commande)}}" class="btn btn-sm btn-primary">
                                            <i class="fas fa-eye"></i>
                                        </a> -->
                                        <a href="{{route('commande.edit',$item->id_commande)}}" class="btn btn-sm btn-warning mx-2">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form id="{{$item->id_commande}}"  action="{{route('commande.destroy',$item->id_commande)}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                        <button type="submit" onclick="deleteForm({{$item->id_commande}})" class="btn btn-sm btn-danger">
